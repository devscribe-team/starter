...
All the custom MDX components available for your documentation pages.
...
Use `<Table>` components for structured data with styled headers, borders, and hover states.
  ...
  <Table>
    ...
    <TableBody>
      ...
      <TableRow>
        ...
        <TableCell>No</TableCell>
        <TableCell>One of `admin`, `editor`, `viewer`</TableCell>
      </TableRow>
      <TableRow>
...
Use `<Column>` to arrange content in a responsive grid. Accepts `cols` (1–4) and `gap` (`sm`, `md`, `lg`).

<TabGroup>
  <Tab title="Preview">
  <Column cols={2}>
  <Card title="Left Column" icon="layout">
  Content in the first column. Cards, text, code blocks, and other components all work inside columns.
  </Card>

  <Card title="Right Column" icon="layers">
  Content in the second column. The grid is responsive — columns stack on smaller screens.
  </Card>
  </Column>

  <Column cols={3} gap="sm">
  <Callout type="info">
  First column callout.
  </Callout>

  <Callout type="warning">
  Second column callout.
  </Callout>

  <Callout type="success">
  Third column callout.
  </Callout>
  </Column>
  </Tab>
